 ...
 *
 * Sep 2026, round 85 — line-coach/development-coach hiring lived here, out
 * of match, so a hire (unlike round 84's mid-match `LineCoachPanel.tsx` card)
 * can't retroactively affect a match already underway. Tyler's own fix,
 * verbatim: "Coaches should not be able to be hired mid-match, and if they
 * are they should only apply from the next match onwards."
 *
 * Round 122 — [[Club Finance, Facilities, and Marketing]] part 2: that
 * hiring UI relocated to the new Football Dept. screen's Assistant Coaches
 * tab (`AssistantCoaches.tsx`), alongside the Talent Scout (round 97 had
 * already moved that one to the Talent Scouting tab). The round-85 rule
 * above is completely unchanged — only which screen hosts the dropdown
 * moved, not when a hire takes effect.
 */
export function SelectionCommittee() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="card text-xs text-slate-400">
        Line Coaches, the Development Coach, and the Talent Scout are hired from the Football Dept.
        screen's Assistant Coaches tab (round 122 — [[Club Finance, Facilities, and Marketing]]
        relocated the hiring UI that used to live here; nothing about how or when a hire takes effect
        changed, only where you go to make one).
      </div>
    </div>
  );
}
